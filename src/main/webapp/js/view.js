...
function updateForm(num) { //num : 수정할 댓글 글번호
	...
	$num.find('.btn-cancel').css('display', 'bolck');
...
$(function() {
	...
	$('ul+.comment-write .btn-register').click(function() {
		...
		//입력한 글 카운트 초기화
		$('.comment-write-area-cotun').text('0/200'); //textarea 초기화
...
/*
	//작성 영역 엔터의 갯수 별로  영역 확보
	var height = $('.comment-write-area-text').css('height');// 17px
	// 댓글을 작성하면서 엔터를 누르면 height가 17px씩 증가합니다.
	$('.comment-area').on('keyup','.comment-write-area-text', function() {
		var entercount = $('.comment-write-area-text').val().split('\n').length;
		height = entercount * 17 + 'px';
